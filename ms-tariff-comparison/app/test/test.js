...
describe("service costs", () => {
  ...
  describe("exception", () => {
    ...
    it('should gives exception by format file', async () => {
      let fileModel = mocks.uploadFileToProcess;
...
describe("service products", () => {
  ...
  describe('exception', () => {
    ...
    it('should not save new product', async () => {
      let product = mocks.createNewProduct;
      product.month = false;
      product.values.maxConsumption = null;
      return await productsService.createProduct(mocks.createNewProduct)
          .catch(reason => expect(reason.message).toEqual("Max consumption KWH is a required field when is a year charge!"));
    });
